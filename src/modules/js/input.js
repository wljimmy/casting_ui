...
function CUIInputInit() {
    ...
    function addLabel(wrapper, labelText, position) {
        ...
        labelEl.className = 'CUI-input__label text-md text-primary';
    ...
    function beautifyInput(input) {
        if (beautifiedElements.has(input)) return;

        const inputType = input.type;

        if (inputType === 'checkbox' || inputType === 'radio') {
            if (!input.classList.contains('CUI-input')) return;
            ...
            const labelHtml = labelText
                ? `<span class="CUI-input__label text-md text-primary">${labelText}</span>`
                : '';

            const hintHtml = hintText
                ? `<span class="CUI-input__hint text-sm text-muted">${hintText}</span>`
                : '';

            const infoHtml = infoText
                ? `<span class="CUI-input__info"><div class="message message-info"><div>${infoText}</div></div></span>`
                : '';

            const errorHtml = errorText
                ? `<span class="CUI-input__error"><div class="message message-error"><div>${errorText}</div></div></span>`
                : '';

            const boxHtml = `
<div class="CUI-input-box${isError ? ' is-error' : ''}">
    ${labelHtml}
    ...
    function processInput(input) {
        ...
        let path = parentId ? `${parentId}.${input.name}` : input.name;
        ...
        if (inputType === 'checkbox' || inputType === 'radio') {
            ...
            input.addEventListener('input', () => {
                const node = CUIInputRegistry.get(path);
                if (node && node.element) node.value = input.value;
                CUIInputRegistry._notifyChange();
            });
        }
    }
